...
class AddRecipeViewModel @Inject constructor(
    ...
    fun submit(onSuccess: () -> Unit, onError: (Throwable) -> Unit) {
        viewModelScope.launch {
            ...
            try {
                val s1 = _step1Fields.value
                val s2 = _step2Fields.value
                val s3 = _step3Fields.value
                val s4 = _step4Fields.value
                val s5 = _step5Fields.value
                // Create recipe without ID first (use 0 as placeholder)
                val recipe = com.nidhi.recipevault.domain.model.Recipe(
                    recipeId = 0, // Will be generated by use case
                    name = s1.name,
                    description = s1.description.ifBlank { null },
                    prepTimeMinutes = s1.prepTimeMinutes.toIntOrNull(),
                    cookTimeMinutes = s1.cookTimeMinutes.toIntOrNull(),
                    servings = s2.servings.toIntOrNull(),
                    difficultyLevel = s2.difficultyLevel,
                    mealType = s2.mealType,
                    cuisine = s2.cuisine,
                    thumbnail = s5.imageUri,
                    ingredients = s3.ingredients.map { it.copy(recipeId = 0) }, // Placeholder
                    methodSteps = s4.methodSteps
                        .filter { it.stepDescription.trim().isNotEmpty() }
                        .sortedBy { it.stepOrder }
                        .mapIndexed { index, step ->
                            step.copy(recipeId = 0, stepOrder = index + 1) // Placeholder
                        }
                )

                // Use case will generate ID and update recipe/ingredients/methodSteps
